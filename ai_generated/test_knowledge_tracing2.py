...
# --------------------------------------------------------------------------
# Cold start vs existing users
# --------------------------------------------------------------------------

def test_cold_start_selects_only_the_root():
    t = KnowledgeTracer(sql_graph())
    assert t.select_next() == "select"


def test_selection_is_pure_function_of_state():
    # No observation between calls -> identical answer. The selector holds no
    # state of its own; the belief vector fully determines it.
    t = KnowledgeTracer(sql_graph())
    assert t.select_next() == t.select_next()


def test_resume_from_events_matches_live_tracer():
    g = sql_graph()
    events = [
        ("select", True), ("select", True), ("where", True),
        ("group_by", False), ("group_by", True), ("joins", True),
    ]
    live = KnowledgeTracer(g)
    for cid, correct in events:
        live.observe(cid, correct=correct)

    resumed = KnowledgeTracer.from_events(g, events)
    assert resumed.beliefs() == pytest.approx(live.beliefs())
    assert resumed.select_next() == live.select_next()


def test_resume_from_state_snapshot_drives_selection():
    # An existing user with mastered basics; the snapshot alone should open the
    # next frontier (here, the dependents of the mastered concepts).
    g = sql_graph()
    snapshot = {"select": 0.95, "where": 0.95}
    t = KnowledgeTracer.from_state(g, snapshot)
    nxt = t.select_next()
    assert nxt is not None
    # Whatever it picks must have all prerequisites satisfied by the snapshot.
    assert all(t.mastery(p) >= 0.6 for p in g.prerequisites(nxt))


def test_preview_path_from_zero_is_foundations_first():
    t = KnowledgeTracer(sql_graph())
    path = t.preview_path(max_steps=9)
    # Roadmap from the zero state must start at the root and respect prereqs.
    assert path[0] == "select"
    pos = {cid: i for i, cid in enumerate(path)}
    for cid in path:
        for p in t.graph.prerequisites(cid):
            if p in pos:
                assert pos[p] < pos[cid]


def test_preview_path_from_populated_state_excludes_mastered():
    t = KnowledgeTracer.from_state(sql_graph(), {"select": 0.95, "where": 0.95})
    path = t.preview_path(max_steps=9)
    assert "select" not in path and "where" not in path
    assert len(path) > 0
