...
- **`backend/services/llm/`** — Single-purpose LLM agents (see TECH_SPEC §3.4) plus a shared `client.py`. `scenario_generator.py` (Agent 1) generates questions; `diagnostic_evaluator.py` (Agent 2) grades free-text answers. **Key pattern: every LLM call has a deterministic mock fallback** — when `OPENAI_API_KEY` is empty, it returns mock output so the whole app runs offline. Preserve this when adding agents. OpenAI is imported lazily inside the call, and uses `client.beta.chat.completions.parse` with Structured Outputs.
- **`backend/api/routes/`** — One router per resource (`domains`, `users`, `assessments`, `recommendations`), each with its own `prefix` and `tags`. Endpoints are listed in TECH_SPEC §5.

Domain logic service packages (TECH_SPEC §3.3 and §3.5):
- **`services/graph/`** — pure in-memory `ConceptGraph` (`Concept` dataclass + DAG traversal: ancestors/descendants/topological order/prerequisites). No DB access.
- **`services/learner/`** — BKT mastery math. `mastery.py` (`apply_diagnostic_result`, `_PARAMS`, prerequisite-gated priors), `bkt.py`/`bkt_models.py`, and `tracer.py`'s `KnowledgeTracer` (belief vector rehydrated via `from_state`/`from_events`; leverage scoring for `recommend_next_to_study`/`recommend_path`).
- **`services/recommender/`** — `recommend_next(user_id, db)` (exported from the package): builds the `ConceptGraph` from the DB, rehydrates a `KnowledgeTracer` from `LearnerState`, picks the highest-leverage learnable concept, maps it to an `action_type` via the Remediation → Exploration → Improvement priority layer (collapsed onto the `assess`/`review` `ActionType` enum), and audits the decision to the `recommendations` table.
